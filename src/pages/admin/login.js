...
const LoginForm = () => {
  ...
  const generateOtp = async () => {
    ...
    try {
      ...
      const templateName = "OTP1"; // optional, if you have a custom template
      const fullPhoneNumber = "+91" + phoneNumber;

      const response = await axios.get(
        `https://2factor.in/API/V1/${apiKey}/SMS/${fullPhoneNumber}/AUTOGEN3/${templateName}`
      );

      if (response.data.Status === "Success") {
        setOtpSent(true);
        setOtpMessage("OTP sent successfully");
        setOrderId(response.data.Details); // store session ID for verification
        setActiveStep(1);
        setError("");
      } else {
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://2factor.in/API/V1/${apiKey}/SMS/VERIFY3/${fullPhoneNumber}/${otp}`
      );
